...
graph_t *SetupGraph(int nvtxs, int *xadj, int *adjncy, int *vwgt, int *adjwgt)
{
    int i, j, k, sum;
    float *nvwgt;
    graph_t *graph;

    /* allocate memory for the adjacency info */
    graph = CreateGraph();
    ...
    graph->xadj   = malloc((nvtxs + 1) * sizeof(int));
    graph->adjncy = malloc(xadj[nvtxs] * sizeof(int));

    IntCopy(nvtxs + 1, xadj, graph->xadj);
    IntCopy(xadj[nvtxs], adjncy, graph->adjncy);
    ...
    if (vwgt) {
        ...
        vwgt = graph->vwgt = malloc(nvtxs * sizeof(int));
        IntSet(nvtxs, 1, vwgt);
    }
    ...
    if (adjwgt) {
        ...
        adjwgt = graph->adjwgt = malloc(graph->nedges * sizeof(int));
        IntSet(graph->nedges, 1, adjwgt);
    }

    /* setup graph label */
    if (graph->label == NULL)
        graph->label = malloc(graph->nvtxs * sizeof(int));
